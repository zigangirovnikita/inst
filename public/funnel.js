(function funnelScreen() {
  ...
  function renderChoiceButtons(plan) {
    ...
    return '<div class="funnel-select-panel"><h2>Какой вариант выбираем?</h2>' + (plan.choices || []).map(choice => {
      const selected = choice.key === plan.selectedChoiceKey;
      return '<button class="choose-funnel ' + (selected ? 'selected' : '') + '" data-choice="' + esc(choice.key) + '">' + esc(labels[choice.key] || choice.label) + '. Дал ' + number(choice.scenario.revenue) + ' ₽<span>' + esc(choice.scenario.title) + (selected ? ' · выбрано' : '') + '</span><span class="choose-funnel-action">' + (selected ? 'Выбрано' : 'Выбрать') + '</span></button>';
    }).join('') + '</div>';
  }
