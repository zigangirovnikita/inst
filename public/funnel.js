(function funnelScreen() {
  ...
  window.renderFunnelResult = function renderFunnelResult() {
    ...
    app.innerHTML = '<section class="screen"><button class="back" id="back-to-errors">← К анализу ошибок</button><h1>Если Reels наберут 1 000 000 просмотров</h1><p class="lead">Представим, что за 30 дней твои Reels получили ' + number(plan.startViews) + ' просмотров. ' + percent(plan.inboxRate * 100) + '% напишут в Direct — это ' + number(plan.startViews * plan.inboxRate) + ' сообщений.</p><p class="funnel-note">Симулятор перебрал ' + number(plan.evaluatedVariants) + ' совместимых ' + variantWord(plan.evaluatedVariants) + ' и показывает три решения: самый простой запуск, оптимальный сейчас и максимум по выручке.</p><h2>Варианты воронки</h2>' + renderChoices(plan) + renderChoiceButtons(plan) + '<ul class="funnel-assumptions"><li>Это модель, а не прогноз продаж.</li><li>Расходы на трафик, бота, менеджера и производство материалов не включены.</li><li>Ручная нагрузка ограничена: ' + number(plan.manualFirstDayCapacity) + ' новых диалогов в первые сутки и ' + number(plan.monthlyCallCapacity) + ' созвонов в месяц.</li></ul></section>';
