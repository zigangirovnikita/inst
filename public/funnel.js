(function funnelScreen() {
  ...
  function renderChoices(plan) {
    return (plan.choices || []).map(choice => {
      ...
      return '<article class="funnel-choice ' + (selected ? 'selected' : '') + '"><div class="funnel-choice-top"><span class="funnel-choice-label">' + esc(choice.label) + '</span><h2>' + esc(choice.scenario.title) + '</h2><p>' + esc(choice.description) + '</p></div>' + renderBusinessSummary(choice.scenario) + '<details class="funnel-details"><summary>Показать конверсии и этапы</summary>' + renderMetrics(choice.scenario) + renderStages(choice.scenario.stages) + '</details></article>';
    }).join('');
  }
  ...
  window.renderFunnelResult = function renderFunnelResult() {
    ...
    app.innerHTML = '<section class="screen"><button class="back" id="back-to-errors">← К анализу ошибок</button><h1>Если Reels наберут 1 000 000 просмотров за месяц</h1><p class="lead">Представим, что за 30 дней твои Reels получили ' + number(plan.startViews) + ' просмотров: примерно ' + number(plan.dailyViews || plan.startViews / 30) + ' просмотров в день. ' + percent(plan.inboxRate * 100) + '% напишут в Direct — это ' + number(plan.startViews * plan.inboxRate) + ' сообщений за месяц.</p><p class="funnel-note">Симулятор считает каждый день отдельно: входящие без ответа остывают в тот же день, а в конце 30 дневных результатов складываются.</p><h2>Варианты воронки</h2>' + renderChoices(plan) + renderChoiceButtons(plan) + '<ul class="funnel-assumptions"><li>Это модель, а не прогноз продаж.</li><li>Расходы на трафик, бота, менеджера и производство материалов не включены.</li><li>Ручная нагрузка считается без переноса на следующий день: максимум ' + number(plan.dailyChatCapacity || 30) + ' переписок и ' + number(plan.dailyCallCapacity || 5) + ' созвонов в день.</li><li>Автоматизированные варианты принимают входящие сразу; дневное ограничение применяется к ручным перепискам и созвонам.</li></ul></section>';
