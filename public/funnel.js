(function funnelScreen() {
  ...
  window.renderFunnelResult = function renderFunnelResult() {
    ...
    app.innerHTML = '<section class="screen"><button class="back" id="back-to-errors">← К анализу ошибок</button><h1>Если завтра твой Reels наберёт 1 000 000 просмотров</h1><p class="lead">Представим, что один ролик резко получил ' + number(plan.startViews) + ' просмотров. ' + percent(plan.inboxRate * 100) + '% напишут в Direct — это ' + number(plan.startViews * plan.inboxRate) + ' сообщений в первые 3 дня, пока люди ещё горячие.</p><p class="funnel-note">Симулятор перебрал ' + number(plan.evaluatedVariants) + ' совместимых ' + variantWord(plan.evaluatedVariants) + ' и показывает три решения: самый простой запуск, оптимальный сейчас и максимум по выручке.</p><h2>Варианты воронки</h2>' + renderChoices(plan) + renderChoiceButtons(plan) + '<ul class="funnel-assumptions"><li>Это модель, а не прогноз продаж.</li><li>Расходы на трафик, бота, менеджера и производство материалов не включены.</li><li>Ручная нагрузка считается только в окне ' + number(plan.responseWindowDays || 3) + ' дня: максимум ' + number(plan.dailyChatCapacity || 30) + ' переписок и ' + number(plan.dailyCallCapacity || 6) + ' созвонов в день.</li><li>Автоматизированные варианты принимают входящие сразу; ограничение применяется к ручным перепискам и созвонам.</li></ul></section>';
