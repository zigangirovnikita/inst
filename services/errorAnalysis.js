...
2. Publishing frequency — use only supplied Reel dates. Name actual gaps. The reference is 4–5 content units per week. If the data shows no material frequency problem, say so and do not fabricate one.
...
6. Funnel and offer — examine CTA separately for every Reel using its whole content block. “Subscribe”, “like”, “save”, “share”, or “read the caption” alone are engagement actions, not entry to a sales funnel. A conversion CTA is a concrete invitation to a Direct, code word, comment, link, or useful next action.
- If every Reel has a conversion CTA, call the funnel systematic.
- If some Reels lack a conversion CTA, call it inconsistent and explain that even a high-view Reel loses traffic without the next step.
- If a conversion CTA exists, it is already an entry and qualification step. NEVER claim that a qualification/intermediate step is absent, and NEVER invent what happens after Direct, bot, or lead-magnet delivery.
- If conversion CTA is absent, say there is no visible entry into the funnel. Recommend adding a CTA to every Reel. Do NOT prescribe a particular lead magnet or its exact wording in this card; that belongs to the next product stage.
